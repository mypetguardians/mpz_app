...
25. **GHCR 빌드 환경변수 3곳 동기화** — NEXT_PUBLIC_* 변경 시 Dockerfile ARG, deploy.yml build-args, docker-compose environment 3곳 모두 맞춰야 함. 하나라도 빠지면 빌드 시 undefined
26. **검색엔진 색인은 대기 필요** — SEO 코드 작업 후 실제 검색 노출까지 수일~수주 소요. 네이버 서치어드바이저/Google Search Console에서 수집 요청 후 기다려야 함
