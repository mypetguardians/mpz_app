...
30. **useEffect 초기 마운트 vs 변경 구분** — sessionStorage 정리 등 부수효과를 useEffect에 넣을 때, 초기 마운트에서도 실행되는지 반드시 확인. 뒤로가기 복원 등과 충돌 가능
31. **CSS 애니메이션은 compositor 속성 우선** — opacity/transform 사용. margin/padding 변경은 layout shift 유발
32. **스크롤 복원 시 sessionStorage 즉시 삭제 금지** — 복원 성공(`scrollTop > 0`) 확인 후에만 삭제. 부모 컴포넌트 리렌더로 자식이 재마운트되면 첫 시도가 리셋될 수 있고, 미리 삭제하면 재시도 불가
33. **레이아웃 영향 요소는 useLayoutEffect로 측정** — 스크롤 복원에 영향을 주는 요소(sticky 영역 높이, negative margin 등)의 크기 측정은 useEffect가 아닌 useLayoutEffect 사용. useEffect는 페인트 후 실행되므로 측정 전에 scrollTo가 먼저 실행되어 위치가 어긋남
34. **버추얼 스크롤 measureElement vs 고정 높이** — 카드 높이가 균일하면 measureElement 사용 금지. estimateSize만으로 충분하고 scrollTop 복원도 정확. measureElement는 높이가 동적일 때만 사용하되, scrollTop 복원이 부정확해지는 트레이드오프 인지 필수
35. **프로그래밍적 scrollTo와 스크롤 방향 감지 충돌** — scrollTo()는 scroll 이벤트를 발생시킴. 스크롤 방향 감지 훅에서 큰 delta(>300px)는 무시해야 복원 시 hide/show가 오작동하지 않음
