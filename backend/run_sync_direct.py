...
async def run_sync():
    """공공데이터 동기화 실행"""
    # 환경 변수 확인
    service_key = os.getenv('PUBLIC_DATA_SERVICE_KEY') or getattr(settings, 'PUBLIC_DATA_SERVICE_KEY', None)
    
    if not service_key:
        print("❌ PUBLIC_DATA_SERVICE_KEY가 설정되지 않았습니다.")
        print("💡 환경 변수를 설정하거나 .env 파일에 추가해주세요.")
        print("   예: export PUBLIC_DATA_SERVICE_KEY='your_key_here'")
        return
    ...
    # 동기화 전 통계
    before_count = Animal.objects.filter(is_public_data=True).count()
    print(f"📊 동기화 전 공공데이터 동물 수: {before_count}개")
    ...
    try:
        ...
        try:
            ...
            print(f"⚠️ 상태 체크 중 오류 (무시): {e}")
        
        # 최종 통계
        print("\n" + "=" * 60)
        print("📊 최종 통계")
        print("=" * 60)
        after_count = Animal.objects.filter(is_public_data=True).count()
        print(f"📊 동기화 후 공공데이터 동물 수: {after_count}개")
